    ...
    <section className="relative min-h-[78vh] overflow-hidden bg-char text-cream">
      ...
      <div className="relative mx-auto grid min-h-[78vh] max-w-7xl content-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8">
        <div className="max-w-2xl">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/menu" className="inline-flex items-center justify-center gap-2 rounded-md bg-ember px-6 py-4 font-black text-white transition hover:bg-mustard hover:text-char">Ver menu <ArrowRight size={18} /></Link><Link href="/orders" className="inline-flex items-center justify-center rounded-md border border-cream/25 px-6 py-4 font-black text-cream transition hover:bg-cream hover:text-char">Mis ordenes</Link></div>
        </div>
        <div className="hidden self-end lg:block"><div className="grid grid-cols-3 gap-3 rounded-lg border border-cream/15 bg-char/70 p-3 backdrop-blur"><Stat icon={<Timer size={18} />} label="Pedido rapido" value="3 pasos" /><Stat icon={<Medal size={18} />} label="Fidelidad" value="10%" /><Stat icon={<Flame size={18} />} label="Menu vivo" value="Realtime" /></div></div>
      </div>
    </section>
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="mb-7 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-black uppercase text-ember">La carta</p><h2 className="text-3xl font-black sm:text-4xl">La mejor parte empieza aqui</h2></div><Link href="/menu" className="inline-flex items-center gap-2 rounded-md bg-ember px-5 py-3 font-black text-white hover:bg-char">Entrar al menu <ArrowRight size={18} /></Link></div>
